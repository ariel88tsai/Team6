...
//bubble & vegetables interaction  
  ...
  if(B_gameTimer<=0){gameState=COOKING_GAME_START;}
    break;
    ...
    if (meetGirl.available()) {meetGirl.read();}
    meetGirl.play(); 
    image(meetGirl, 0, 0, width, height);
    float mdM = meetGirl.duration();
    float mtM = meetGirl.time();
    if(mtM == mdM){gameState = D_GAME_START;} 
    break;
